...
except Exception:  # pragma: no cover (tests can import without Streamlit)
    st = None  # type: ignore
...
try:
    from shapely.geometry import LineString, Point  # noqa: F401 (presence check only)
    HAVE_SHAPELY = True
except Exception:
    HAVE_SHAPELY = False
...
def _parse_timestamp_from_block(block: str) -> Optional[datetime]:
    ...
    # ISO-like date time, e.g. 2025-09-27 10:10:10 or with 'T'
...
# Default, case‑insensitive patterns. NOTE: single backslashes in raw strings.
DEFAULT_PATTERNS = {
    "lat": [
...
def parse_srt(
    file_bytes: bytes,
    file_name: str,
    custom_regex: Optional[Dict[str, List[str]]] = None,
) -> Tuple[List[SrtRecord], List[str]]:
    """
    Robust SRT parser. Returns (records, warnings).
    - Uses multiple fallback regexes.
    - Skips blocks missing lat/lon/time.
    - If timestamp missing, fabricates monotonic seconds from index.
    """
    ...
    patterns: Dict[str, List[re.Pattern]] = {k: [] for k in DEFAULT_PATTERNS}
    if custom_regex:
        for key, plist in custom_regex.items():
            if key in patterns:
                for ptxt in plist:
                    try:
                        patterns[key].append(re.compile(ptxt, re.IGNORECASE))
    ...
    idx_counter = 1
    for block in blocks:
        # Optional numeric index at top of block
        ...
        if not tstamp:
            warnings.append(
                f"{file_name}: A block had coordinates but no absolute timestamp — using index order."
            )
...
def parse_kml_first_linestring(file_bytes: bytes) -> Tuple[List[Tuple[float, float]], List[str]]:
    """Parse KML, return first LineString as a list of (lat, lon)."""
    warnings: List[str] = []
    try:
...
def aeqd_transformer_for_chain(coords_latlon: List[Tuple[float, float]]):
    ...
    Returns forward (lat,lon)->(x,y) and inverse (x,y)->(lat,lon) callables.
    """
    ...
    if HAVE_PYPROJ:
        ...
        # Return as (lat,lon)->(x,y) for convenience
        return (lambda lat, lon: fwd(lon, lat), lambda x, y: inv(x, y))
    else:
        # Fallback: equirectangular approximation about the centroid
        R = 6371008.8
        ...
        def inv_fallback(x: float, y: float):
            lat = math.degrees(y / R + lat0r)
            lon = math.degrees(x / (R * math.cos(lat0r)) + lon0r)
            return lat, lon
...
def chain_segments_xy(chain_latlon: List[Tuple[float, float]]):
    """Precompute XY arrays and segment vectors for the chain."""
...
def project_points_to_chain(
    points_latlon: List[Tuple[float, float]],
    chain_latlon: List[Tuple[float, float]],
):
    """
    For each point, find nearest projection on the chain and return arrays:
    ...
    for i, p in enumerate(P):
        AP = p - A  # vector from each segment start to point
...
def kml_flight_style(i: int) -> str:
    ...
    return (
        f"<Style id='flight{i}'><LineStyle><color>{color}</color><width>3</width></LineStyle></Style>\n"
    )
...
def merge_flights_in_order(
    flights: List[Flight],
) -> Tuple[List[SrtRecord], List[Tuple[int, int, int]]]:
    """
    Merge flights preserving selected order; reindex starting at 1.
    Returns merged records and a list of (start_idx, end_idx, flight_i) per original.
    """
    ...
    for fi, f in enumerate(flights):
        ...
        for r in f.records:
            merged.append(
                SrtRecord(index=cur, time=r.time, lat=r.lat, lon=r.lon, alt=r.alt, raw_block=r.raw_block)
            )
            cur += 1
        end = cur - 1
...
def records_to_dataframe(
    recs: List[SrtRecord], chainage_m: np.ndarray, dist_m: np.ndarray
) -> pd.DataFrame:
    df = pd.DataFrame(
        {
...
def dataframe_to_srt(df: pd.DataFrame) -> str:
    """Minimal SRT export with augmented info."""
    lines = []
    for _, row in df.iterrows():
        ...
        text = (
            f"[latitude: {row['lat']:.6f}] [longitude: {row['lon']:.6f}] [altitude: {row['alt']:.2f}] "
...
def build_kml(
    ...
    kml = [
        kml_document_header(
            description="Creator: Vijay Parmar — for BGol (Community of Surveyors and GIS Experts)"
        )
    ]
    ...
    kml.append(
        "<Placemark><name>Chain</name><styleUrl>#chainStyle</styleUrl>"
        f"<LineString><tessellate>1</tessellate><coordinates>\n{chain_coords}\n</coordinates></LineString></Placemark>\n"
    )

    # Per‑flight lines + start/end placemarks
    for i, f in enumerate(flights):
        start_idx, end_idx, _ = boundaries[i]
        coords = [(r.lat, r.lon, r.alt if not math.isnan(r.alt) else 0.0) for r in f.records]
    ...
    if include_merged and len(merged_df):
        coords = list(
            zip(
                merged_df["lat"].tolist(),
                merged_df["lon"].tolist(),
                merged_df["alt"].fillna(0).tolist(),
            )
        )
...
def run_app():
    ...
    if kml_file is not None:
        ...
        # Chain length in km via projection distances
        _, _, _, L, _, _ = chain_segments_xy(chain_latlon)
    ...
    with st.expander("Advanced: Custom regex (optional)"):
        ...
        custom_txt = st.text_area(
            "Custom regex JSON",
            height=120,
            placeholder='{"lat": ["(?i)Latitude\\s*[:=]\\s*([\\-0-9.]+)"]}',
        )
        custom_regex = None
        if custom_txt.strip():
            try:
    ...
    flights.sort(key=lambda F: (F.earliest or datetime.max))  # default by earliest
    ...
    exclude_missing = st.checkbox(
        "Exclude blocks missing lat/lon from outputs", value=True
    )  # parser already skips; kept for UI parity
    ...
    if st.button("Compute Chainage (merge & project)"):
        with st.spinner("Computing projections and chainage..."):
            ...
            # Plotly preview (3D: lon, lat, alt)
            import plotly.graph_objects as go

            fig = go.Figure()
            fig.add_trace(
                go.Scatter3d(
                    x=[c[1] for c in chain_latlon],
                    y=[c[0] for c in chain_latlon],
                    z=[0] * len(chain_latlon),
                    mode="lines",
                    name="Chain",
                    line=dict(width=4),
                )
            ...
            kml_text = build_kml(
                chain_latlon, ordered_flights, merged_df, boundaries, include_merged=include_merged_kml
            )
            ...
            zbuf.seek(0)
            st.download_button(
                "Download all outputs (ZIP)", data=zbuf.getvalue(), file_name="outputs.zip", mime="application/zip"
            )
            ...
            if not (HAVE_PYPROJ and HAVE_SHAPELY):
                st.info(
                    "Accurate projection libraries missing — results may have increased error; please install `pyproj` & `shapely` for meter-accurate projections."
                )


# Run UI only when launched via `streamlit run`
